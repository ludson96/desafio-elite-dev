    ...
    <div className="max-w-md w-full space-y-8 bg-zinc-900 border border-zinc-800 p-8 rounded-3xl shadow-2xl">
      ...
      <div className="space-y-2 pt-4 border-t border-zinc-800">
        ...
        <div className="grid grid-cols-3 gap-2">
          <button
            type="button"
            onClick={() => fillQuickAccount("cliente1@eliteingressos.com")}
          ...
          <button
            type="button"
            onClick={() => fillQuickAccount("organizador@eliteingressos.com")}
            className="p-2 rounded-lg bg-zinc-950 border border-zinc-800 hover:border-zinc-700 text-[11px] text-zinc-300 font-medium transition-colors"
          >
            👑 Organizador
          </button>
          <button
            type="button"
            onClick={() => fillQuickAccount("portaria@eliteingressos.com")}
            className="p-2 rounded-lg bg-zinc-950 border border-zinc-800 hover:border-zinc-700 text-[11px] text-zinc-300 font-medium transition-colors"
          >
            🚪 Portaria
          </button>
...
export default function LoginPage() {
  return (
    <div className="flex-1 flex items-center justify-center py-12 px-4 sm:px-6 lg:px-8">
      <Suspense fallback={<div className="h-96 w-full max-w-md bg-zinc-900/60 rounded-3xl animate-pulse" />}>
        <LoginForm />
      </Suspense>
    </div>
  );
